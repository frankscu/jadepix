...
int process_events_add(string filename="J:\\data\\test_180208113927.df", 
                       string outfile_name ="J:\\data\\HB_test_180208113927.root", 
                       int skip_time_flag=2)
{
  ...
  if (skip_time_flag == 1)
  {
    int time_size = 35 * sizeof(unsigned int);
    file.seekg(-time_size, std::ios::end);
  }
  else
  {
  ...
  if (skip_time_flag == 1)
  {
    file.seekg(time_size, std::ios::beg); // skip the time stame
    file.seekg(SKIP_FRAMES * size_of_frame, std::ios::cur); // skip the first 100 frames
  }
  else
  {
  ...
  for (unsigned int n_frame = 0; n_frame <= max_frames; n_frame++)
  {
        ...
        for (unsigned int p = 0; p < NBUFFER; p++)
        {
          ...
          for (unsigned int pk = 0; pk < BUFFER_SIZE; pk++)
          {
            for (unsigned int i = 0; i < NROW; i++)
            {
              ...
              if (TMath::Abs(cds_proc_frames_ADC[p * BUFFER_SIZE + pk][i]) > 200)
              { // physical signal suppressed
    ...
    else
    {
      ...
      for (unsigned int i = 0; i < NROW; i++)
      {
        ...
        if ((TMath::Abs(cds_frame_ADC[i]) < PIX_SEED_TH))
          continue;
        ...
        else
        {
          ...
          if ((TMath::Abs(cds_frame_ADC[i]) > TMath::Abs(cds_frame_ADC[i - 1])) && (TMath::Abs(cds_frame_ADC[i]) > TMath::Abs(cds_frame_ADC[i + 1])))
          {
            ...
            if (TMath::Abs(cds_frame_ADC[i - 1]) > PIX_CLUS_TH)
              tmp_clus_val += TMath::Abs(cds_frame_ADC[i - 1]);
            if (TMath::Abs(cds_frame_ADC[i + 1]) > PIX_CLUS_TH)
              tmp_clus_val += TMath::Abs(cds_frame_ADC[i + 1]);
