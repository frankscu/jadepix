...
JadeDataFrame::JadeDataFrame(std::string &&data)
  :m_data_raw(std::move(data)), m_is_decoded(false),
   ...
   m_n_x(0), m_n_y(0),
   m_cds_frame_adc({0}),
   m_pix_seed_thr(500),m_pix_clus_thr(200),
   m_cluster_size(3)
{
}
...
void JadeDataFrame::SetFrameCDS(std::vector<int16_t>& cds_frame_adc){
  m_cds_frame_adc = cds_frame_adc;
}
...
std::map<std::pair<size_t, size_t>, int16_t> JadeDataFrame::GetFrameSeed(){
  return m_seed;
}

std::map<std::pair<size_t, size_t>, int16_t> JadeDataFrame::CalFrameSeed(std::vector<int16_t>& _cds_frame_adc){
  std::map<std::pair<size_t, size_t>, int16_t> _seed;
  for(size_t iy = 0 ; iy < m_n_y; iy++){
    for(size_t ix = 0 ; ix < m_n_x; ix++){
      if(IsInEdge(ix, iy)){
        continue;
      }
      auto _seed_val = _cds_frame_adc.at(iy+m_n_y*ix);
      if(_seed_val > m_pix_seed_thr){
        _seed.insert(std::make_pair(std::make_pair(ix, iy), _seed_val));
      }
    }
  }
  return _seed;
...
std::vector<int16_t> JadeDataFrame::GetFrameCluster(){
  m_cluster.clear();
  for(auto& seed : m_seed){
    int16_t _pix_clus_adc = 0;
    
    for(size_t iy=seed.first.first-int(m_cluster_size/2); iy<=seed.first.first+int(m_cluster_size/2);iy++)
    {
      for(size_t ix=seed.first.second-int(m_cluster_size/2); ix<=seed.first.second+int(m_cluster_size/2);ix++){
        if(IsInMatrix(ix, iy)){
          if(m_cds_frame_adc.at(iy+m_n_y*ix) > m_pix_clus_thr) {
            _pix_clus_adc += m_cds_frame_adc.at(iy+m_n_y*ix);
          }
        }
      }
    }
    m_cluster.push_back(_pix_clus_adc);
  }  
  return m_cluster;
}
...
JadeDataFrame JadeDataFrame::operator-(JadeDataFrame &df){
  ...
  std::transform(df.GetFrameData().begin(), df.GetFrameData().end(), m_data.begin(), m_cds_frame_adc.begin(), std::minus<int16_t>());
  return *this; 
}
