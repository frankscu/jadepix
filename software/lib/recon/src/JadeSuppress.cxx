...
void JadeSuppress::OpenInputFile(string filein){
...
void JadeSuppress::OpenOutputFile(string fileout){
...
int JadeSuppress::ReadEvent(JadeEvent* evt){
...
int JadeSuppress::WriteEvent(JadeEvent* evt){
  WriteEventTag(m_evtId);
  //Write McTruth
  int nofMc=evt->NofTruth();
  WriteMcTag(nofMc);
  for(int iMc=0;iMc<nofMc;iMc++){
    JadeHit* truthHit = evt->GetTruthHit(iMc);
    WriteMc(truthHit);
  }
  ...
  for(int iDigi=0;iDigi<nofDigi;iDigi++){
    JadeDigi* digi = evt->GetDigi(iDigi);
...
void JadeSuppress::WriteEventTag(int id){
  (*m_fout)<<""<<endl;
  (*m_fout)<<"EventId: "<<id<<endl;
}

void JadeSuppress::WriteMc(JadeHit* truthHit){
...
void JadeSuppress::WriteMcTag(int n){
  (*m_fout)<<"McTruth Hit No: "<<n<<endl;
}


void JadeSuppress::WriteDigiTag(int n){
  (*m_fout)<<"Digi Hit No: "<<n<<endl;
}

void JadeSuppress::WriteDigi(JadeDigi* digi){
